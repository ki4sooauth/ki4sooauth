...
public class ShopUserRegisterCoreServiceImpl implements ShopUserRegisterCoreService
{
    ...
    public boolean register(ShopInfo shopInfo, ShopUserInfo shopUserInfo) throws Exception
    {
        ...
        synchronized (synObject)
        {
            ...
            String roleId = bundle.getString("register_default_role");
            ShopUserRole2 shopUserRole2 = new ShopUserRole2();
            shopUserRole2.setShopId(shopInfo.getShopId());
            shopUserRole2.setRoleId(roleId);
            shopUserRole2.setIsDel("N");
            if (!this.shopUserRole2GeneratorCoreService.insertSelective(shopUserRole2))
            {
                GooagooLog.error("商家用户注册：添加默认权限（" + shopUserRole2.toString() + "）异常", null);
                throw new OperateFailException("商家用户注册：添加默认权限（" + shopUserRole2.toString() + "）异常");
            }
        }
        return true;
    }

...
public class UserLoginCoreServiceImpl implements UserLoginCoreService
{
    ...
    public PersonalLoginInfo login(String account, String password, UserMobileInfo userMobileInfo, Integer expireSecond) throws Exception
    {
        ...
        if (userMobileInfo != null)
        {
            ...
            //8、清除mac-user缓存
            RedisDatabase redisDatabase = new RedisDatabase(RedisServerConstants.business_mac);
            redisDatabase.del(userMobileInfo.getMacAddress());
        }
        else
        {
            //8、组装用户登录信息
            ...
            UserMobileInfo mobileInfo = this.userMobileInfoGeneratorCoreService.selectByPrimaryKey(userInfo.getUserId());
            if (mobileInfo != null)
            {
                PersonalMobileInfo personalMobileInfo = new PersonalMobileInfo();
        ...
        //10.清除缓存
        RedisDatabase redisDatabase = new RedisDatabase(RedisServerConstants.business_user);
        redisDatabase.del(userInfo.getUserId());

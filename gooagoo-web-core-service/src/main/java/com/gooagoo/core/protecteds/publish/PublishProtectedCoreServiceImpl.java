...
public class PublishProtectedCoreServiceImpl implements PublishProtectedCoreService
{
    ...
    public boolean generateHtml(Object obj) throws Exception
    {
        ...
        OutputStreamWriter pcOut = new OutputStreamWriter(new FileOutputStream(pcFile), "UTF-8");
        OutputStreamWriter mobileOut = new OutputStreamWriter(new FileOutputStream(mobileFile), "UTF-8");
    ...
    private void replaceGoodsInfo(GoodsBaseInfo goodsBaseInfo, String pcContent, String mobileContent) throws Exception
    {
        ...
        {
            StringBuffer sb = new StringBuffer();
            sb.append("<section><div class='productIntroduce'><h3>商品介绍：<span id='moreInt'>更多描述></span></h3><div class='content'>" + goodsCache.get("goodsContent") + "</div></div></section>");
        ...
        {
            shopEntityLocation = "<a href='http://html." + UrlUtils.getBASE_HOST() + "/gus/merchant/mobile/html/baidumap.html?x=" + shopEntityCache.get("shopGpsBaidux") + "&y=" + shopEntityCache.get("shopGpsBaiduy") + "' class='sectionA'><span class='icon-location-2'></span><span class='icon-location-4'></span>" + shopEntityCache.get("shopRoadGuide") + "</a>";
        }
        ...
        .replaceAll("\\$\\{data.imgPath \\}", "http://html." + UrlUtils.getBASE_HOST())//静态资源路径
        .replaceAll("\\$\\{data.basePath \\}", "http://3g." + UrlUtils.getBASE_HOST() + "/mm");//应用访问路径
    }
    ...
    {
        System.out.println(new File("F:/out/com/fetchview/mobile/entity/moba01/transform/Integral.j").exists());
    }
}

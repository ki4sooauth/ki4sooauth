...
import com.gooagoo.api.business.core.statistics.ShareStatisticCoreService;
...
@Service
public class ShareStatisticCoreServiceImpl implements ShareStatisticCoreService
{
    /**
     * 判断是否是今天
     * @param dateTime
    ...
    public int couponShareTimes(String couponId, String userType, String dateType, Date dateTime)
    {
        ...
        if (this.isToday(dateTime))
        {
            ...
            String valueSource = this.jointValueForRedis(couponId, "S", userType, dateType, dateTime);
    ...
    public List<String> couponSharePeople(String couponId, String userType, String dateType, Date dateTime)
    {
        ...
        if (this.isToday(dateTime))
        {
            //redis 查询商家所有实体店店内人群
            RedisSortedSetDao sortedSetDao = new RedisSortedSetDao(RedisServerConstants.statistics_coupon);
            String key = this.jointKeyForRedis(couponId, "S", dateType, userType, dateTime);

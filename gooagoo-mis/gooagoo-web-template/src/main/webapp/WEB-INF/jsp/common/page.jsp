...
<div class="shopping_Flip">
	...
	<!-- 小数计算时会有误差，所以装换成整数在计算 -->
		<a href="javascript:void(0);" class="flip_downBtn blueBtn" 
			<c:choose>
				<c:when test="${pm.count % 10 == 0}">onclick="page(${pm.count / 10});"</c:when>
				<c:otherwise>onclick="page(${(pm.count+pm.pageSize-1)/pm.pageSize});"</c:otherwise>
			</c:choose>>末&nbsp;&nbsp;页</a>
	</c:if>
	<c:if test="${page_cur != page_end }">
		<a href="javascript:void(0);" class="flip_downBtn blueBtn" onclick="page(${page_cur + 1 });">下一页</a>
	</c:if>
	...
	<c:if test="${page_cur != 1}">
		<a href="javascript:void(0);" class="flip_upBtn blueBtn" onclick="page(${page_cur - 1 });">上一页</a>
	</c:if>
	<c:if test="${page_cur != 1 }">
		<a href="javascript:void(0);" class="flip_upBtn blueBtn" onclick="page(1);">首&nbsp;&nbsp;页</a>
	</c:if>
</div>

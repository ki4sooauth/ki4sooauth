...
<%
	String nowDate = TimeUtils.getCurrentDateTime();
	request.setAttribute("currentTimeS", nowDate);
	request.setAttribute("currentTimeE", TimeUtils.dateAdd(4,nowDate,1));
%>
			...
			<ul style="width: 500px;">
				<li><span style="width: 140px;"><code>*</code>广告位编码：</span>
					...
					<c:forEach items="${pm.result }" var="item">
						<option value="${item.adCode }">${item.adCode }-${item.adName }</option>
					</c:forEach>
					</select>
				</li>
				<li><span style="width: 140px;"><code>*</code>起拍价：</span>
					<input type="text" class="name_input" id="startingPrice" name="startingPrice" />
				...
				<li><span style="width: 140px;"><code>*</code>涨幅：</span>
					<input type="text" class="name_input" id="increase" name="increase" />
				...
				<li><span style="width: 140px;"><code>*</code>竞价起始时间：</span>
					<input id="bidStartTime" name="bidStartTime" type="text" onfocus="WdatePicker({dateFmt:'yyyy-MM-dd HH:mm:ss',minDate:'${currentTimeS}'})" class="Wdate" style="cursor: pointer;" />
				...
				<li><span style="width: 140px;"><code>*</code>竞价结束时间：</span>
					<input id="bidEndTime" name="bidEndTime" type="text" onfocus="WdatePicker({dateFmt:'yyyy-MM-dd HH:mm:ss',minDate:'${currentTimeE}'})" class="Wdate" style="cursor: pointer;" />
				...
				<li><span style="width: 140px;"><code>*</code>生效起始日期：</span>
					<input id="effectStartDate" name="effectStartDate" type="text" onfocus="WdatePicker({dateFmt:'yyyy-MM-dd'})" class="Wdate" style="cursor: pointer;" />
...
function saveDic() {
	if (checkStartingPrice() && checkIncrease() && checkBidStartTime() && checkBidEndTime() && checkEffectStartDate() && checkEffectEndDate() && checkEffectStartTime() && checkEffectEndTime()) {
		var url = "adManage.do?method=addAdManage";

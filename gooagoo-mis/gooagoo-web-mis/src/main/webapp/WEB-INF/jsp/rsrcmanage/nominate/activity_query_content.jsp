...
function cancel(){
	...
	if(confirm("是否取消推荐此商品？")){
		...
		ajaxJsonTipByData("nominateA.do?method=cancelNominateActivity",data,true);
		page(1);
	}
}
...
function update(){
	...
	if(checkOnly()){
		var url = "nominateA.do?method=updateActivityPage&id=" + ids;
...
//新增推荐
function add(){
	var url = "nominateA.do?method=quertConditionPage";
	$("#sysuserFancybox2").attr("href",url).click();
}
</script>
			...
			<c:when test="${!empty pm.result}">
				<c:forEach var="item" items="${pm.result }" varStatus="im" >
					<tr>
						...
						<td><fmt:formatDate value="${item.nominateActivityCTimeStamp}" pattern="yyyy-MM-dd" /></td>
					</tr>
				</c:forEach>
			</c:when>
			<c:otherwise>

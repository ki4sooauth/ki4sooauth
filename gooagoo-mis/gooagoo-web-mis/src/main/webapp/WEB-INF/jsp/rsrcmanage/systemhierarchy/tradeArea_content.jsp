...
//新增
function add(){
	var url = "tradeArea.do?method=addTradeAreaPage";
...
function update(){
	...
	if(checkOnly()){
		var url = "tradeArea.do?method=queryTradeAreaInfo&tradeAreaId=" + ids;
		$("#sysuserFancybox2").attr("href",url).click();
	}
}
...
function del(){
	...
	if(confirm("是否将选择的信息删除？")){
		var data = "pageIndex=1&tradeAreaId=" + ids;
		ajaxJsonTipByData("tradeArea.do?method=deleteTradeArea",data,true);
		page(1);
	}
}
</script>
...
<div class="list_box_search" style="padding:0 0 0 1px;overflow-x:auto;width:100%;white-space:nowrap">
	<table width="100%" border="0" cellspacing="0" cellpadding="0" class="searchForm" border="1">
		...
		<c:forEach var="item" items="${pm.result }" varStatus="im" >
			<tr>
				<td><input type="checkbox" name="single" value="${item.tradeAreaId }" />&nbsp;${im.count + ((page_cur-1) * 10)}</td>
				<td>
					<c:forEach items="${platList }" var="plat">
						<c:if test="${item.platformId eq plat.platformId}">${plat.platformName }</c:if>
					</c:forEach>
				</td>

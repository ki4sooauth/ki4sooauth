...
function cancel(){
	...
	if(confirm("是否取消推荐此优惠凭证？")){
		var data = "pageIndex=1&id=" + ids;
		ajaxJsonTipByData("nominateC.do?method=cancelNominateCoupon",data,true);
		page(1);
	}
}
...
function update(){
	...
	if(checkOnly()){
		var url = "nominateC.do?method=updateCouponPage&id=" + ids;
...
//新增推荐
function add(){
	var url = "nominateC.do?method=quertConditionPage";
	$("#sysuserFancybox2").attr("href",url).click();
}
</script>
</head>
			...
			<c:when test="${!empty pm.result}">
				<c:forEach var="item" items="${pm.result }" varStatus="im" >
					<tr>
						...
						<td><fmt:formatDate value="${item.nominateCouponCTimeStamp}" pattern="yyyy-MM-dd" /></td>
					</tr>
				</c:forEach>
			</c:when>
		<c:otherwise>

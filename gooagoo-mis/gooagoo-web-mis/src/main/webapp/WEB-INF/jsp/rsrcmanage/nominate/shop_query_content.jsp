...
function cancel(){
	...
	if(confirm("是否取消推荐此商家？")){
		var data = "pageIndex=1&id=" + ids;
		ajaxJsonTipByData("nominateS.do?method=cancelNominateShop",data,true);
		page(1);
	}
}
...
function update(){
	...
	if(checkOnly()){
		var url = "nominateS.do?method=updateShopPage&id=" + ids;
...
//新增推荐
function add(){
	var url = "nominateS.do?method=quertConditionPage";
	$("#sysuserFancybox2").attr("href",url).click();
}
			...
			<c:when test="${!empty pm.result}">
				<c:forEach var="item" items="${pm.result }" varStatus="im" >
					<tr>
						...
						<td><fmt:formatDate value="${item.nominateShopCTimeStamp}" pattern="yyyy-MM-dd" /></td>
					</tr>
				</c:forEach>
			</c:when>
		<c:otherwise>

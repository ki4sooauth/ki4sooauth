...
<body>
	<form id="addForm" method="post">
		...
		<p>
		<div style="padding:0 0 0 360px">起始时间： 
			<input id="startTime" name="startTime" type="text" value="<fmt:formatDate value="${coupon.startTime}" type="both" pattern="yyyy-MM-dd HH:mm:ss" />"
				onclick="WdatePicker({dateFmt:'yyyy-MM-dd HH:mm:ss',minDate:'${currentTime}'})" class="Wdate" style="cursor: pointer;" /> 
			<label class="wrong" id="lblstartTime" style="display: none;">请选择起始时间！</label>
		</div>
		<br>
		<div style="padding:0 0 0 360px">结束时间： 
			<input id="endTime" name="endTime" type="text" value="<fmt:formatDate value="${coupon.endTime}" type="both" pattern="yyyy-MM-dd HH:mm:ss" />"
				class="Wdate" onclick="WdatePicker({dateFmt:'yyyy-MM-dd HH:mm:ss',minDate:'${currentTime}'})" style="cursor: pointer;" /> 
			<label class="wrong" id="lblendTime" style="display:none;">请选择结束时间！</label>
		</div>
		<div style="padding:41px 0 0 430px">
			<input style="width:70px;height:24px;" id="save" type="button" value="保存" onclick="enter()" />
		</div>
		<input name="id" type="hidden" value="${coupon.id }" />
		<input name="couponId" type="hidden" value="${coupon.couponId }" />
	</form>
	<script type="text/javascript">
		...
		function enter() {
			...
			if (checkForm()) {
				var url = "nominateC.do?method=updateNominateCoupon";

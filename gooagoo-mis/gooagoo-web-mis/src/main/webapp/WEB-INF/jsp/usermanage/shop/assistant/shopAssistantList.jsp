...
function del(){
	if("" == ids || ids == null){
		...
		if(flag){
			if(confirm("确定删除选中信息？")){
				var url = "shopAssistant.do?method=delAssistant";
				var data = "&ids=" + ids;
	...
	if("" == ids || ids == null){
		...
		if(checkOnly()){
			if($("#isDel"+ids.split(",")[0]).text().trim() == "否"){
				...
				var url = "shopAssistant.do?method=updateAssistantpcPage&ids=" + ids;
		...
		<c:choose>
			...
			<c:when test="${isDel eq 'Y'}">
				<c:if test="${item.pId eq parentId3 and item.name eq '编辑'}">
					<input type="button" value="编辑" onclick="updateAssistant()" />
				</c:if>
			</c:when>
			<c:otherwise>
				<c:if test="${item.pId eq parentId3 and item.name eq '删除'}">
...
<table width="100%"  cellspacing="0" cellpadding="0" class="searchForm" border="1">
	...
	<tr>
		<td><input type="checkbox" name="single" value="${item.deviceAssistantId }"/>&nbsp;${im.count + ((page_cur-1) * 10)}</td>
		...
		<td id="isDel${item.deviceAssistantId}">
		<c:choose>

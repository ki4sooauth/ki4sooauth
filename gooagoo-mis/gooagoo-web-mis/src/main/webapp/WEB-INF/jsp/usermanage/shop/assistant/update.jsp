...
<div class="popMsg">
	...
	<ul>
		<li>
			...
			<select id="shopId" name="shopId">
				<c:forEach var="itemShop" items="${shopInfoList }" varStatus="imShop" >
				    <option value="${itemShop.shopId}" <c:if test="${deviceAssistant.shopId eq itemShop.shopId}">selected="selected"</c:if>>${itemShop.shopName }</option>
				</c:forEach>
			</select>
		</li>
		<li>
			...
			<select id="shopEntityId" name="shopEntityId">
				<c:forEach var="itemShopEntity" items="${mShopEntityInfoList }" varStatus="imShopEntity" >
				   <option value="${itemShopEntity.shopEntityId}" <c:if test="${deviceAssistant.shopEntityId eq itemShopEntity.shopEntityId}">selected="selected"</c:if>>${itemShopEntity.shopEntityName }</option>
				</c:forEach>
			</select>
		...
		<li>
			<span>MAC地址：</span>
			${deviceAssistant.deviceMac }
		</li>
		<li>
			<span>设备型号：</span>
			<c:choose>
				<c:when test="${deviceAssistant.deviceType eq '0'}">普通型</c:when>
				<c:when test="${deviceAssistant.deviceType eq '1'}">精简型</c:when>
			</c:choose>
		</li>
		<li>
			<span>设备序列号：</span>
			${deviceAssistant.deviceSn }
		</li>
		<li>
			<span>安装日期：</span>
			<input id="installDate" name="installDate"  class="Wdate" type="text" onfocus="WdatePicker()" value="<fmt:formatDate value="${deviceAssistant.installDate}" type="both" pattern="yyyy-MM-dd"/>" style="cursor: pointer;" />
...
<div class="popBtn">
	...
	<input type="hidden" name="deviceAssistantId" value="${deviceAssistant.deviceAssistantId}"/>
</div>
</form>
<script type="text/javascript">
$(function(){
	$("#note").val('${deviceAssistant.note}');
	$("#shopId").bind("change", changeShop);
	var isDel = '${deviceAssistant.isDel }';
...
//根据商家ID查询商家实体店集合
function changeShop(){
	var url = "shopAssistant.do?method=selectShopEntityInfo";
	...
	if(!isEmpty(shopEntityList)){
		...
		$(shopEntityList).each(function(index){
			...
			if("${deviceAssistant.shopEntityId}" == val.shopEntityId){
...
//保存
function save(){
	if(checkNull('installDate','lbinstallDate')  || checkNull('shopEntityId','lbshopEntityId')){
		return false;
	} 
	if(!isEmpty($("#shopId").val())){
		var url = "shopAssistant.do?method=updateAssistantpc";

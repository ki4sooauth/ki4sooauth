...
// 增加
function add(){
	var url = "interRequ.do?method=showDicAdd";
...
function del(){
	...
	if(confirm("是否将此字典信息删除？")){
		...
		ajaxJsonTipByData("interRequ.do?method=delDict",data,true);
		page(1);
	}
}
...
function showUpdateJsp(id){
	var url = "interRequ.do?method=showDicEdit&id="+ id;
	$("#sysuserFancybox2").attr("href",url).click();
}
...
function exportEx(){
	...
	exportExcel('interRequ.do?method=exportExcel',data);
}
</script>
...
<div class="div_but">
	<c:forEach var="item" items="${mis_login_auths}" varStatus="im">
		...
		<c:if test="${item.pId eq parentId2 and item.name eq '导入' }">
			<label title="导入Excel"><input id="importExcel" type="button" value="导入" onclick="importExcel('sysuserFancybox2','interRequ.do?method=importExcel')" /></label>
		</c:if>
...
<div class="list_box_search" style="padding:0 0 0 1px;">
	<table width="100%" border="0" cellspacing="0" cellpadding="0" class="searchForm" border="1">
		<tr bgcolor="red">
			...
			<th width="26%">接口编码</th>
			<th width="20%">参数英文名称</th>
			<th width="25%">参数中文名称</th>
			<th width="12%">是否必填</th>
			<th width="12%">更新时间</th>
		</tr>
		<c:choose>
		...
		<c:forEach var="item" items="${pm.result }" varStatus="im" >
			<tr>
				...
				<td>${item.nameCn }</td>
				<td>
					<c:if test="${item.isRequired eq 'Y' }">是</c:if>
					<c:if test="${item.isRequired eq 'N' }">否</c:if>
				</td>

		...
		<form action="" method="post" id="addForm" name="addForm">
			<ul>
				<li><span><code>*</code>接口编码：</span>
					<input type="hidden" id="id" name="id" value="${interRequest.id}" />
					<input type="text" id="iCode" name="iCode" value="${interRequest.iCode}" />
				...
				<li><span><code>*</code>参数英文名称：</span>
					<input type="text" class="name_input" id="nameEn" name="nameEn" value="${interRequest.nameEn }"/>
					<label class="wrong" id="lblIName" style="display: none;">请输入参数英文名称！</label>
				</li>
				<li><span><code>*</code>参数中文名称：</span>
					<input type="text" class="name_input" id="nameCn" name="nameCn" value="${interRequest.nameCn }" />
					<label class="wrong" id="lblIType" style="display: none;">请输入参数中文名称！</label>
				</li>
				<li>
					<span><code>*</code>是否必填：</span>
					<input type="radio" name="isRequired" class="other_add" value="Y" <c:if test="${interRequest.isRequired eq 'Y' }">checked=checked</c:if> /><label class="other_add_txt">是</label>
					<input type="radio" name="isRequired" class="other_add" value="N" <c:if test="${interRequest.isRequired eq 'N' }">checked=checked</c:if> />/><label class="other_add_txt">否</label>
				</li>
				<li>
...
function saveDic() {
	if (checkMessage() && checkNameEn() && checkNameCn()) {
		var url = "interRequ.do?method=editDict";

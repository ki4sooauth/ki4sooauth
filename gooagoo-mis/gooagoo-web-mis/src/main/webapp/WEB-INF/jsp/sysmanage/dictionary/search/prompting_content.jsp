...
// 增加
function add(){
	var url = "prompdict.do?method=showDicAdd";
...
function del(){
	...
	if(confirm("是否将此字典信息删除？")){
		...
		ajaxJsonTipByData("prompdict.do?method=delDict",data,true);
		page(1);
	}
}
...
function showUpdateJsp(id){
	var url = "prompdict.do?method=showDicEdit&id="+ id;
	$("#sysuserFancybox2").attr("href",url).click();
}
...
function exportEx(){
	...
	exportExcel('prompdict.do?method=exportExcel',data);
}
</script>
...
<div class="div_but">
	<c:forEach var="item" items="${mis_login_auths}" varStatus="im">
		...
		<c:if test="${item.pId eq parentId2 and item.name eq '导入' }">
			<label title="导入Excel"><input id="importExcel" type="button" value="导入" onclick="importExcel('sysuserFancybox2','prompdict.do?method=importExcel')" /></label>
		</c:if>
...
<div class="list_box_search" style="padding:0 0 0 1px;">
	<table width="100%" border="0" cellspacing="0" cellpadding="0" class="searchForm" border="1">
		...
		<c:forEach var="item" items="${pm.result }" varStatus="im" >
			<tr>
				<td><input type="checkbox" name="single" value="${item.messageId }" />&nbsp;${im.count + ((page_cur-1) * 10)}</td>
				<td>${item.messageId }</td>
				<td><span class="lengthHide" title="${item.content}">${item.content }</span></td>
				<td>${item.sys }</td>
				<td>${item.func }</td>
				<td><fmt:formatDate value="${item.cTimeStamp}" pattern="yyyy-MM-dd" /></td>
				<td><span class="lengthHide" title="${item.note}">${item.note }</span></td>
			</tr>
		</c:forEach>
		</c:when>
		<c:otherwise>

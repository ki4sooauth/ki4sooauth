...
// 增加
function add(){
	var url = "shopType.do?method=showDicAdd";
...
function del(){
	...
	if(confirm("是否将此字典信息删除？")){
		...
		ajaxJsonTipByData("shopType.do?method=delDict",data,true);
		page(1);
	}
}
...
function showUpdateJsp(id){
	var url = "shopType.do?method=showDicEdit&id="+ id;
	$("#sysuserFancybox2").attr("href",url).click();
}
...
function exportEx(){
	...
	exportExcel('shopType.do?method=exportExcel',data);
}
</script>
...
<div class="div_but">
	<c:forEach var="item" items="${mis_login_auths}" varStatus="im">
		...
		<c:if test="${item.pId eq parentId2 and item.name eq '导入' }">
			<label title="导入Excel"><input id="importExcel" type="button" value="导入" onclick="importExcel('sysuserFancybox2','shopType.do?method=importExcel')" /></label>
		</c:if>
...
<div class="list_box_search" style="padding:0 0 0 1px;">
	<table width="100%" cellspacing="0" cellpadding="0" class="searchForm" border="1">
		<tr bgcolor="red">
			...
			<th width="25%">商家类型名称</th>
			<th width="17%">上级类型编号</th>
			<th width="22%">点击前图片</th>
			<th width="22%">点击后图片</th>
			<th width="12%">更新时间</th>
		</tr>
		<c:choose>
		...
		<c:forEach var="item" items="${pm.result }" varStatus="im" >
			<tr>
				<td><input type="checkbox" name="single" value="${item.shopTypeId }" />&nbsp;${im.count + ((page_cur-1) * 10)}</td>
				<td>${item.shopTypeName }</td>
				<td>${item.parentShopTypeId }</td>

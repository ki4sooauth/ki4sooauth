...
// 增加
function add(){
	var url = "sysdict.do?method=showDicAdd";
...
function del(){
	...
	if(confirm("是否将此字典信息删除？")){
		...
		ajaxJsonTipByData("sysdict.do?method=delDict",data,true);
		page(1);
	}
}
...
function showUpdateJsp(id){
	var url = "sysdict.do?method=showDicEdit&id="+ id;
	$("#sysuserFancybox2").attr("href",url).click();
}
...
function exportEx(){
	...
	exportExcel('sysdict.do?method=exportExcel',data);
}
</script>
...
<div class="div_but">
	<c:forEach var="item" items="${mis_login_auths}" varStatus="im">
		...
		<c:if test="${item.pId eq parentId2 and item.name eq '导入' }">
			<label title="导入Excel"><input id="importExcel" type="button" value="导入" onclick="importExcel('sysuserFancybox2','sysdict.do?method=importExcel')" /></label>
		</c:if>
...
<div class="list_box_search" style="padding:0 0 0 1px;">
	<table width="100%" border="0" cellspacing="0" cellpadding="0" class="searchForm" border="1">
		...
		<c:forEach var="item" items="${pm.result }" varStatus="im" >
			<tr>
				<td><input type="checkbox" name="single" value="${item.sysDictionaryId }" />&nbsp;${im.count + ((page_cur-1) * 10)}</td>

...
// 增加
function add(){
	var url = "market.do?method=showDicAdd";
...
function del(){
	...
	if(confirm("是否将此字典信息删除？")){
		...
		ajaxJsonTipByData("market.do?method=delDict",data,true);
		page(1);
	}
}
...
function showUpdateJsp(id){
	var url = "market.do?method=showDicEdit&id="+ id;
	$("#sysuserFancybox2").attr("href",url).click();
}
...
function exportEx(){
	...
	exportExcel('market.do?method=exportExcel',data);
}
</script>
...
<div class="div_but">
	<c:forEach var="item" items="${mis_login_auths}" varStatus="im">
		...
		<c:if test="${item.pId eq parentId2 and item.name eq '导入' }">
			<label title="导入Excel"><input id="importExcel" type="button" value="导入" onclick="importExcel('sysuserFancybox2','market.do?method=importExcel')" /></label>
		</c:if>
...
<div class="list_box_search" style="padding:0 0 0 1px;">
	<table width="100%" border="0" cellspacing="0" cellpadding="0" class="searchForm" border="1">
		<tr bgcolor="red">
			...
			<th width="18%">营销渠道编码</th>
			<th width="22%">营销渠道名称</th>
			<th width="23%">父级编码</th>
			<th width="20%">排序号</th>
			<th width="12%">更新时间</th>
		</tr>
		...
		<c:forEach var="item" items="${pm.result }" varStatus="im" >
			<tr>
				<td><input type="checkbox" name="single" value="${item.channelCode }" />&nbsp;${im.count + ((page_cur-1) * 10)}</td>
				<td>${item.channelCode }</td>
				<td>${item.channelName }</td>
				<td>${item.parentChannelCode }</td>

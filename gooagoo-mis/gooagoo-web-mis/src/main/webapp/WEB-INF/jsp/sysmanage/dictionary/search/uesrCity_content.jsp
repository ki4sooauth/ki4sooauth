...
// 增加
function add(){
	var url = "userCity.do?method=showDicAdd";
...
function del(){
	...
	if(confirm("是否将此字典信息删除？")){
		...
		ajaxJsonTipByData("userCity.do?method=delDict",data,true);
		page(1);
	}
}
...
function showUpdateJsp(id){
	var url = "userCity.do?method=showDicEdit&id="+ id;
	$("#sysuserFancybox2").attr("href",url).click();
}
...
function exportEx(){
	...
	exportExcel('userCity.do?method=exportExcel',data);
}
</script>
...
<div class="div_but">
	<c:forEach var="item" items="${mis_login_auths}" varStatus="im">
		...
		<c:if test="${item.pId eq parentId2 and item.name eq '导入' }">
			<label title="导入Excel"><input id="importExcel" type="button" value="导入" onclick="importExcel('sysuserFancybox2','userCity.do?method=importExcel')" /></label>
		</c:if>
	...
	<table width="100%" border="0" cellspacing="0" cellpadding="0" class="searchForm" border="1">
		...
		<c:forEach var="item" items="${pm.result }" varStatus="im" >
			<tr>
				...
				<td>${item.cityId }</td>
				<td>${item.cityName }</td>
				<td>${item.parentCityId }</td>
				<td>${item.patentCityName }</td>
				<td>
					<c:choose>
						<c:when test="${item.levelCode eq 'P' }">省</c:when>
						<c:when test="${item.levelCode eq 'C' }">市</c:when>

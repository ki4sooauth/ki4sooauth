...
public class PositionZip
{
    ...
    {
        byte[] bytes = new byte[behaviors.size() * 71];
        for (int i = 0; i < behaviors.size(); i++)
        {
            byte[] single = new byte[71];
            ...
            System.arraycopy(behavior.getEntityId().getBytes(), 0, single, 39, 32);//32字节实体店id 总计71字节

            System.arraycopy(single, 0, bytes, i * 71, 71);
        }
    ...
    public static List<Behavior> toBehavior(String input) throws IOException
    {
        ...
        for (int i = 0; i < (bytes.length) / 71; i++)
        {
            Behavior behavior = new Behavior();
            byte[] single = new byte[71];
            System.arraycopy(bytes, i * 71, single, 0, 71);
            ...
            behavior.setEntityId(new String(single, 39, 32));//区域id

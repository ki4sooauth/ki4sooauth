...
public class BrowseActiveForAccount implements Customer
{
    @Override
    public void message(Object message)
    {
        GooagooLog.debug("启动任务 BrowseActive");
        try
        {
            BehaveLog behaveLog = (BehaveLog) message;
            String activeId = behaveLog.getObjectValue();
            if ("A".equals(behaveLog.getObjectType()) && StringUtils.hasText(activeId))
            {
                ActivityQueryService activityQueryService = SpringBeanUtils.getBean(ActivityQueryService.class);
                ActivityDetail activityDetail = activityQueryService.findActivityDetail(activeId);
        ...
        {
            GooagooLog.error("BrowseActive", e);
        }
    }
}

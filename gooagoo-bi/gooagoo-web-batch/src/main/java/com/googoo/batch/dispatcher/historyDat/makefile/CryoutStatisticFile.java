...
public class CryoutStatisticFile implements Tyre
{
    private final MongoDao dao = new MongoDao(MongoConstants.MONGO_URL, MongoConstants.makefileDB, CollectionConstants.CRYOUT_FILE_INFO);
    ...
    private void saveFile(String id, List<Integer> brows, List<String> dateList)
    {
        ...
        String chartStr = chartVo.create("吆喝内容", "人次", "次", dateList, dataMap);
        String fileName = FileUtils.getfileName(id);
        FileUtils.createAndWriteFile(BatchConsants.CRYOUT, chartStr, fileName);
    }

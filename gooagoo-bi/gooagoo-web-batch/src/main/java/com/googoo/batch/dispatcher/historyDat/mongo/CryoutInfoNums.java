...
public class CryoutInfoNums implements Tyre
{
    private final RedisSortedSetDao redisSortedSetDao = new RedisSortedSetDao(RedisServerConstants.statistics_cryout);
    private final MongoDao mongoDao_num = new MongoDao(MongoConstants.MONGO_URL, MongoConstants.cryoutDB, CollectionConstants.CO_CRYOUT_NUMS);

    @Override
    public void run()
    {
        GooagooLog.debug("启动任务 CryoutInfoNums");

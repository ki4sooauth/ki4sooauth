...
public class ShopUserInfoNum implements Tyre
{
    private final RedisDatabase redisDatabase = new RedisDatabase(RedisServerConstants.statistics_member);
    ...
    private final MongoDao mongoDao_num = new MongoDao(MongoConstants.MONGO_URL, MongoConstants.shopUserDB, CollectionConstants.SU_SHOPUSER_NUM);

    @Override
    public void run()
    {
        GooagooLog.debug("启动任务 ShopUserInfoNum");

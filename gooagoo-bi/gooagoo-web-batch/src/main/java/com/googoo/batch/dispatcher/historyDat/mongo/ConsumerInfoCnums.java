...
public class ConsumerInfoCnums implements Tyre
{
    private final RedisSortedSetDao redisSortedSetDao = new RedisSortedSetDao(RedisServerConstants.statistics_consumption);
    private final MongoDao mongoDao_num = new MongoDao(MongoConstants.MONGO_URL, MongoConstants.consumerDB, CollectionConstants.CS_CONSUMER_CNUMS);

    @Override
    public void run()
    {
        GooagooLog.debug("启动任务 ConsumerInfoCnums");
        //查询当天的Redis数据存入MongoDB
        Set<String> values = this.redisSortedSetDao.get("count", 0, -1);
        for (String value : values)
        {
            ...
            if (!DayJudgeUtil.isToday(value))
            {
                String mongoId = "";
                String mongoKey = "";
                Double pnum = this.redisSortedSetDao.getScore("count", value);
                ...
                //this.redisSortedSetDao.delElement("count", value);
            }
        }
    }
}

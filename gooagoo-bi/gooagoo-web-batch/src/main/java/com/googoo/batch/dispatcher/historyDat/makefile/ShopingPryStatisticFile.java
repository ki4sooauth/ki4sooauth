...
public class ShopingPryStatisticFile implements Tyre
{
    private final MongoDao dao = new MongoDao(MongoConstants.MONGO_URL, MongoConstants.makefileDB, CollectionConstants.SHOPINGPRY_FILE_INFO);
    ...
    private void statistic(String id, DBObject browses)
    {
        ...
        if (id.endsWith("H"))
        {
            ...
            this.saveFile(id, brows, dateList);
        }
    ...
    private void saveFile(String id, List<Integer> brows, List<String> dateList)
    {
        Map<String, List<?>> dataMap = new HashMap<String, List<?>>();
        dataMap.put("浏览", brows);
        LineChartVo chartVo = new LineChartVo();
        String chartStr = chartVo.create("购好奇内容", "人次", "次", dateList, dataMap);
        String fileName = FileUtils.getfileName(id);
        FileUtils.createAndWriteFile(BatchConsants.INQUISITIVE, chartStr, fileName);
    }

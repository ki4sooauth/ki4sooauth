...
public class PhoneToolStatisticFile implements Tyre
{
    private final MongoDao dao = new MongoDao(MongoConstants.MONGO_URL, MongoConstants.makefileDB, CollectionConstants.PHONETOOL_FILE_INFO);
    ...
    public void run()
    {
        ...
        for (DBObject obj : all)
        {
            ...
            this.statistic(id, browses);
        }
    }

    private void statistic(String id, DBObject browses)
    {
        List<Integer> browse = new ArrayList<Integer>();
        ...
        if (id.endsWith("H"))
        {
            ...
            for (int i = 1; i <= hour; i++)
            {
                ...
                dateList.add(i + "时");
                browse = this.getNums(browses, browse, "H", date);
            }
            this.saveFile(id, browse, dateList);
        }
        else if (id.endsWith("D"))
        {
            ...
            for (int i = 1; i <= day; i++)
            {
                ...
                dateList.add(i + "日");
                browse = this.getNums(browses, browse, "D", date);
            }
            this.saveFile(id, browse, dateList);
        }
        else if (id.endsWith("M"))
        {
            ...
            for (int i = 1; i <= month; i++)
            {
                ...
                dateList.add(i + "月");
                browse = this.getNums(browses, browse, "M", date);
            }
            this.saveFile(id, browse, dateList);
        }
        else if (id.endsWith("Y"))
        {
            ...
            for (int i = year - 5; i <= year; i++)
            {
                ...
                dateList.add(i + "年");
                browse = this.getNums(browses, browse, "Y", date);
            }
            this.saveFile(id, browse, dateList);
        }
    }

    private void saveFile(String id, List<Integer> use, List<String> dateList)
    {
        ...
        FileUtils.createAndWriteFile(BatchConsants.PHONETOOL, chartStr, fileName);
    }

...
public class NoticeStatisticFile implements Tyre
{
    private final MongoDao dao = new MongoDao(MongoConstants.MONGO_URL, MongoConstants.makefileDB, CollectionConstants.NOTICE_FILE_INFO);
    ...
    private void saveFile(String id, List<Integer> brows, List<String> dateList)
    {
        ...
        String chartStr = chartVo.create("通知内容", "人次", "次", dateList, dataMap);
        String fileName = FileUtils.getfileName(id);
        FileUtils.createAndWriteFile(BatchConsants.NOTICE, chartStr, fileName);
    }

...
public class VoucherInfoNums implements Tyre
{
    private final RedisSortedSetDao redisSortedSetDao = new RedisSortedSetDao(RedisServerConstants.statistics_coupon);
    private final MongoDao mongoDao_num = new MongoDao(MongoConstants.MONGO_URL, MongoConstants.voucherDB, CollectionConstants.VC_VOUCHER_NUMS);

    @Override
    public void run()
    {
        GooagooLog.debug("启动任务 VoucherInfoNums");

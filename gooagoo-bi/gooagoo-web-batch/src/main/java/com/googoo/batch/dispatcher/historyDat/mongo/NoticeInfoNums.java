...
public class NoticeInfoNums implements Tyre
{

    private final RedisSortedSetDao redisSortedSetDao = new RedisSortedSetDao(RedisServerConstants.statistics_notice);
    private final MongoDao mongoDao_num = new MongoDao(MongoConstants.MONGO_URL, MongoConstants.noticeDB, CollectionConstants.NT_NOTICE_NUMS);

    @Override
    public void run()
    {
        GooagooLog.debug("启动任务 NoticeInfoNums");

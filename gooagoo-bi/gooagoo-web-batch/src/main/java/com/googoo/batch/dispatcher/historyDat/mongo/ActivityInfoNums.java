...
public class ActivityInfoNums implements Tyre
{
    private final RedisSortedSetDao redisSortedSetDao = new RedisSortedSetDao(RedisServerConstants.statistics_activity);
    private final MongoDao mongoDao_num = new MongoDao(MongoConstants.MONGO_URL, MongoConstants.activityDB, CollectionConstants.ACT_ACTIVITY_NUMS);

    @Override
    public void run()
    {
        GooagooLog.debug("启动任务 ActivityInfoNums");
        ...
        for (String value : values)
        {
            ...
            if (!DayJudgeUtil.isToday(value))
            {
                ...
                if (k.size() == 6)
                {
                    ...
                    mongoKey = k.get(2) + "_" + k.get(1) + "_" + k.get(3) + "_" + k.get(4);
                }
                else
                {
                    GooagooLog.warn("Redis数据库的key值不合法！");
                }
                if (mongoId.length() > 1 && mongoKey.length() > 3)
                {

...
public class NoticeInfoNum implements Tyre
{

    private final RedisDatabase redisDatabase = new RedisDatabase(RedisServerConstants.statistics_notice);
    private final RedisSortedSetDao redisSortedSetDao = new RedisSortedSetDao(RedisServerConstants.statistics_notice);
    private final GridFSDao gridFSDao = new GridFSDao(MongoConstants.MONGO_URL, MongoConstants.gridFSDB);
    private final MongoDao mongoDao_num = new MongoDao(MongoConstants.MONGO_URL, MongoConstants.noticeDB, CollectionConstants.NT_NOTICE_NUM);

    @Override
    public void run()
    {
        GooagooLog.debug("启动任务 NoticeInfoNum");
        ...
        for (String key : areaInfoNumKeySet)
        {
            if (!DayJudgeUtil.isToday(key))
            {
                ...
                if (keys.size() == 5)
                {
                    ...
                    mongoKey = keys.get(2) + "_" + keys.get(3);
                }
                else
                {

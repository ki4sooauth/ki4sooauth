...
public class InteractionInfoNums implements Tyre
{
    private final RedisSortedSetDao redisSortedSetDao = new RedisSortedSetDao(RedisServerConstants.statistics_interaction);
    private final MongoDao mongoDao_num = new MongoDao(MongoConstants.MONGO_URL, MongoConstants.interactionDB, CollectionConstants.IA_INTERACTION_NUMS);

    @Override
    public void run()
    {
        GooagooLog.debug("启动任务 InteractionInfoNums");

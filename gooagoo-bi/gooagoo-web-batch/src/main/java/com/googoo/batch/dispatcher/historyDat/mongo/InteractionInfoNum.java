...
public class InteractionInfoNum implements Tyre
{
    private final RedisDatabase redisDatabase = new RedisDatabase(RedisServerConstants.statistics_interaction);
    private final RedisSortedSetDao redisSortedSetDao = new RedisSortedSetDao(RedisServerConstants.statistics_interaction);
    private final GridFSDao gridFSDao = new GridFSDao(MongoConstants.MONGO_URL, MongoConstants.gridFSDB);
    private final MongoDao mongoDao_num = new MongoDao(MongoConstants.MONGO_URL, MongoConstants.interactionDB, CollectionConstants.IA_INTERACTION_NUM);

    @Override
    public void run()
    {
        GooagooLog.debug("启动任务 InteractionInfoNum");
        ...
        for (String key : areaInfoNumKeySet)
        {
            if (!DayJudgeUtil.isToday(key))
            {
                ...
                if (keys.size() == 4)
                {
                    ...
                    mongoKey = keys.get(1) + "_" + keys.get(2);
                }
                else
                {

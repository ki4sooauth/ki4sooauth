...
public class ActivityInfoFile implements Tyre
{
    private final RedisSortedSetDao redisSortedSetDao = new RedisSortedSetDao(RedisServerConstants.statistics_activity);
    private final MongoDao activity_file_dao = new MongoDao(MongoConstants.MONGO_URL, MongoConstants.makefileDB, CollectionConstants.ACTIVITY_FILE_INFO);

    @Override
    public void run()
    {
        GooagooLog.debug("启动任务 ActivityInfoFile");
    ...
    private void statistic(String value, String dateType)
    {
        ...
        _id.append(ids[0]).append("_" + ids[2]).append("_" + ids[3]).append("_" + ids[4]).append("_" + dateType);
        ...
        this.activity_file_dao.update(query, new BasicDBObject().append("$set", obj));
    }
}

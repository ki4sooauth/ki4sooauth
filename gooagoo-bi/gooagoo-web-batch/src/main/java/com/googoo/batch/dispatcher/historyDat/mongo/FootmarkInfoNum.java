...
public class FootmarkInfoNum implements Tyre
{
    private final RedisDatabase redisDatabase = new RedisDatabase(RedisServerConstants.statistics_arrive);
    private final RedisSortedSetDao redisSortedSetDao = new RedisSortedSetDao(RedisServerConstants.statistics_arrive);
    private final GridFSDao gridFSDao = new GridFSDao(MongoConstants.MONGO_URL, MongoConstants.gridFSDB);
    private final MongoDao mongoDao_num = new MongoDao(MongoConstants.MONGO_URL, MongoConstants.footmarkDB, CollectionConstants.FM_FOOTMARK_NUM);

    @Override
    public void run()
    {
        GooagooLog.debug("启动任务 FootmarkInfoNum");
        ...
        for (String key : redisBaseKeySet)
        {
            if (!DayJudgeUtil.isToday(key))
            {
                ...
                Set<String> redisNum = this.redisSortedSetDao.get(key, 0, -1);
                this.gridFSDao.save(new ArrayList<String>(redisNum), pid, pid);

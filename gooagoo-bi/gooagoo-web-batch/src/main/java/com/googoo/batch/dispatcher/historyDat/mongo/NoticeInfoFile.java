...
public class NoticeInfoFile implements Tyre
{
    private final RedisSortedSetDao redisSortedSetDao = new RedisSortedSetDao(RedisServerConstants.statistics_notice);
    private final MongoDao notice_file_dao = new MongoDao(MongoConstants.MONGO_URL, MongoConstants.makefileDB, CollectionConstants.NOTICE_FILE_INFO);

    @Override
    public void run()
    {
        GooagooLog.debug("启动任务 NoticeInfoFile");
    ...
    private void statistic(String value, String dateType)
    {
        ...
        this.notice_file_dao.update(query, new BasicDBObject().append("$set", obj));
    }

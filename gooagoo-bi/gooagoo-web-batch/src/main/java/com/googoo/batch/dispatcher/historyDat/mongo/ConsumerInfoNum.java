...
public class ConsumerInfoNum implements Tyre
{
    private final RedisDatabase redisDatabase = new RedisDatabase(RedisServerConstants.statistics_consumption);
    ...
    private final MongoDao mongoDao_num = new MongoDao(MongoConstants.MONGO_URL, MongoConstants.consumerDB, CollectionConstants.CS_CONSUMER_NUM);

    @Override
    public void run()
    {
        GooagooLog.debug("启动任务 ConsumerInfoNum");

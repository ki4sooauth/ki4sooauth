...
public class VoucherInfoNum implements Tyre
{
    private final RedisDatabase redisDatabase = new RedisDatabase(RedisServerConstants.statistics_coupon);
    ...
    private final MongoDao mongoDao_num = new MongoDao(MongoConstants.MONGO_URL, MongoConstants.voucherDB, CollectionConstants.VC_VOUCHER_NUM);

    @Override
    public void run()
    {
        GooagooLog.debug("启动任务 VoucherInfoNum");
        ...
        for (String key : areaInfoNumKeySet)
        {
            if (!DayJudgeUtil.isToday(key))
            {
                ...
                if (keys.size() == 4)
                {
                    ...
                    mongoKey = keys.get(2) + "_" + keys.get(1);
                }
                else if (keys.size() == 6)
                {
                    mongoId = keys.get(0) + "_" + keys.get(keys.size() - 1);

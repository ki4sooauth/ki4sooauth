...
public class CryoutInfoNum implements Tyre
{
    private final RedisDatabase redisDatabase = new RedisDatabase(RedisServerConstants.statistics_cryout);
    ...
    private final MongoDao mongoDao_num = new MongoDao(MongoConstants.MONGO_URL, MongoConstants.cryoutDB, CollectionConstants.CO_CRYOUT_NUM);

    @Override
    public void run()
    {
        GooagooLog.debug("启动任务 CryoutInfoNum");

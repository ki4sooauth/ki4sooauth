...
public class FavoriateMobileServiceImpl implements FavoriateMobileService
{
        ...
        if (null != favoriteLinkInfoBusiness)
        {
            ...
            if (CollectionUtils.isNotEmpty(favoriteLinkInfoBusiness.getFavoriteInfoBusinessList()))
            {
                ...
                for (FavoriteInfoBusiness favoriteInfo : favoriteLinkInfoBusiness.getFavoriteInfoBusinessList())
                {
                    ...
                    favoritelistg.setInfourl(this.getIntroduceUrl(favoriteInfo.getObjectid(), favoriteInfo.getInfotype(), favoriteInfo.getInfourl()));
    ...
    /**
     * 获取手机端详情介绍地址
     * @param infoId 信息编号，如：商品、优惠券、活动
     * @param infoType 信息类型：A-活动，G-商品，C-优惠券
     * @param infoUrl 网站端详细信息介绍地址
     * @return 获取手机端详情介绍地址
     * @throws Exception
     */
    private String getIntroduceUrl(String infoId, String infoType, String infoUrl) throws Exception
    {

        String introduceUrl = "";//详情介绍地址，如商品、优惠券、活动
        if ("A".equals(infoType))
        {
            introduceUrl = UrlUtils.getActiveMobileUrl(infoId);
        }
        else if ("G".equals(infoType))
        {
            introduceUrl = UrlUtils.getGoodsMobileUrl(infoId);
        }
        else if ("C".equals(infoType))
        {
            introduceUrl = UrlUtils.getCouponMobileUrl(infoId);
        }
        String attachString = "";
        if (StringUtils.hasText(infoUrl) && infoUrl.contains("?"))
        {//当前商品、优惠券、活动参与了营销活动
            attachString = infoUrl.substring(infoUrl.indexOf("?"), infoUrl.length());
            introduceUrl = introduceUrl + attachString;
        }
    ...
    public FavoritePlazaRoot getFavoritePlazaListInfo(String userId, String keyWord, String type, String pageIndex, String pageSize) throws Exception
    {
        ...
        if (CollectionUtils.isNotEmpty(favoritePlaceList))
        {
            ...
            for (FavoritePlace favoritePlace : favoritePlaceList)
            {
                ...
                favoriteplaza.setLinkurl(this.getIntroduceUrl(favoritePlace.getObjId(), favoritePlace.getObjType(), favoritePlace.getLinkUrl()));

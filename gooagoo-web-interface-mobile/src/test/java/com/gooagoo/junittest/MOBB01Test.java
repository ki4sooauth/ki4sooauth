...
public class MOBB01Test extends TestCase
{

    String interfaceCode = "mobb01";//接口编码
    ...
    public void testUseridIsNull()
    {
        ...
        root = (FavoriteListgRoot) HttpClientUtils.HttpPost(list, root, this.interfaceCode);
    ...
    public void testNotLogin()
    {
        ...
        list.add(new BasicNameValuePair("sessionid", getLoginInfo.getSessionId()));
        list.add(new BasicNameValuePair("ctimestamp", "2100-10-06"));
        list.add(new BasicNameValuePair("mac", getLoginInfo.getMacAddress()));
        list.add(new BasicNameValuePair("mver", getLoginInfo.getmVer()));
        list.add(new BasicNameValuePair("lid", getLoginInfo.getLid()));
        list.add(new BasicNameValuePair("entityposition", getLoginInfo.getEntityposition()));
    ...
    /**
     * 查询成功（时间戳为null,查询所有用户收藏信息）
     */
    public void testSuccessWithCtimestampIsNull()
    {
        List<NameValuePair> list = new ArrayList<NameValuePair>();//传入参数
        list.add(new BasicNameValuePair("userid", getLoginInfo.getUserId()));
        list.add(new BasicNameValuePair("sessionid", getLoginInfo.getSessionId()));
    ...
    /**
     * 查询成功（时间戳不为null,查询大于当前所给时间戳的用户收藏信息）
     */
    public void testSuccessWithCtimestampIsNotNull()
    {
        List<NameValuePair> list = new ArrayList<NameValuePair>();//传入参数
        list.add(new BasicNameValuePair("userid", getLoginInfo.getUserId()));
        list.add(new BasicNameValuePair("sessionid", getLoginInfo.getSessionId()));
        list.add(new BasicNameValuePair("ctimestamp", "2012-08-01"));
        list.add(new BasicNameValuePair("mac", getLoginInfo.getMacAddress()));
        list.add(new BasicNameValuePair("mver", getLoginInfo.getmVer()));
        list.add(new BasicNameValuePair("lid", getLoginInfo.getLid()));
        FavoriteListgRoot root = new FavoriteListgRoot();
        root = (FavoriteListgRoot) HttpClientUtils.HttpPost(list, root, this.interfaceCode);
        Assert.assertEquals("收藏列表成功", root.getMsg().trim());
    }
}

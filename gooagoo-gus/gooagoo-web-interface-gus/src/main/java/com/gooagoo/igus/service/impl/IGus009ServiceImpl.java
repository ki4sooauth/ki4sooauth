 ...
 *
 */
@Service("igus009Service")
public class IGus009ServiceImpl implements IGusService
{
    ...
    public TransData<Object> service(HttpServletRequest request)
    {
        ...
        try
        {
            ...
            {
                GooagooLog.error("获取最新商品列表：未获取到满足条件的最新商品列表", null);
            ...
            for (NewGoods newGoods : NewGoodsList)
            {
                ...
                ugoods.setGoodsId(newGoods.getGoodsId());
                ugoods.setGoodsName(newGoods.getGoodsName());
                ugoods.setGoodsLink(UrlUtils.getGoodsUrl(newGoods.getGoodsId()));
                if (StringUtils.isNotBlank(newGoods.getGoodsImageUrl()))
                {
                    List<String> imageList = new Gson().fromJson(newGoods.getGoodsImageUrl(), new TypeToken<List<String>>()
                    {
                    }.getType());
                    ugoods.setGoodsImage(FormatDataUtils.formatImageInfo(imageList.get(0)));
                }
                ugoods.setShopId(newGoods.getShopId());
                ugoods.setShopName(newGoods.getShopName());
        ...
        {
            GooagooLog.error("获取最新商品列表：获取最新商品列表异常", e);

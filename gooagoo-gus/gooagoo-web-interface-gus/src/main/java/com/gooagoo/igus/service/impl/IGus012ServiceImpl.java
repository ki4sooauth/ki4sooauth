package com.gooagoo.igus.service.impl;

import java.util.ArrayList;
import java.util.List;

import javax.servlet.http.HttpServletRequest;

import org.apache.commons.collections.CollectionUtils;
import org.springframework.beans.factory.annotation.Autowired;
import org.springframework.stereotype.Service;
import org.springframework.web.bind.ServletRequestUtils;

import com.gooagoo.api.business.query.marketing.analysis.GuessYouLikeQueryService;
import com.gooagoo.common.entity.TransData;
import com.gooagoo.common.gus.constants.Constants;
import com.gooagoo.common.gus.utils.FormatDataUtils;
import com.gooagoo.common.log.GooagooLog;
import com.gooagoo.common.utils.UrlUtils;
import com.gooagoo.entity.business.marketing.analysis.GuessYouLikeActivity;
import com.gooagoo.igus.service.IGusService;
import com.gooagoo.view.gus.UActivity;

/**
 * 获取猜您喜欢活动列表
 * @author SPZ
 *
 */
@Service("igus012Service")
public class IGus012ServiceImpl implements IGusService
{

    @Autowired
    private GuessYouLikeQueryService guessYouLikeQueryService;

    @Override
    public TransData<Object> service(HttpServletRequest request)
    {
        TransData<Object> transData = null;
        try
        {
            String userId = ServletRequestUtils.getStringParameter(request, Constants.USER_LOGIN_USERID);
            Integer pageSize = ServletRequestUtils.getIntParameter(request, "pageSize");
            Integer pageIndex = ServletRequestUtils.getIntParameter(request, "pageIndex");
            List<GuessYouLikeActivity> guessYouLikeActivityList = this.guessYouLikeQueryService.guessYouLikeActivity(userId, pageIndex, pageSize);
            if (CollectionUtils.isEmpty(guessYouLikeActivityList))
            {
                GooagooLog.error("获取猜您喜欢活动列表：未获取到满足条件的猜您喜欢活动列表", null);
                return new TransData<Object>(false, null, null);//TODO 没有数据异常
            }
            List<UActivity> uactivityList = new ArrayList<UActivity>();
            for (GuessYouLikeActivity guessYouLikeActivity : guessYouLikeActivityList)
            {
                UActivity uactivity = new UActivity();
                uactivity.setActivityId(guessYouLikeActivity.getActivityId());
                uactivity.setActivityName(guessYouLikeActivity.getActivityName());
                uactivity.setActivityLink(UrlUtils.getActiveUrl(guessYouLikeActivity.getActivityId()));
                uactivity.setActivityImage(FormatDataUtils.formatImageInfo(guessYouLikeActivity.getActivityImageUrl()));
                uactivity.setShopId(guessYouLikeActivity.getShopId());
                uactivity.setShopName(guessYouLikeActivity.getShopName());
                uactivityList.add(uactivity);
            }
            transData = new TransData<Object>(true, null, uactivityList);
        }
        catch (Exception e)
        {
            GooagooLog.error("获取猜您喜欢活动列表：获取猜您喜欢活动列表异常", e);
            transData = new TransData<Object>(false, null, null);//TODO 系统异常
        }
        return transData;
    }

}

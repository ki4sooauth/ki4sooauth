...
<div class="page_box">
	<c:if test="${pageModel.pageIndex > 1 }">
		<a class="prev arrow" href="javascript:void(0);" onclick="javascript:doPage_goods('${pageModel.pageIndex - 1 }');"></a><!--上一页-->
	</c:if>
	<c:if test="${pageModel.pages > 1}">
		...
		<c:forEach begin="${pageIndexMin }" end="${pageModel.pageIndex }" var="item">
			<c:choose>
				...
				<c:otherwise>
					<a href="javascript:void(0);" onclick="javascript:doPage_goods('${item }');">${item }</a>
				</c:otherwise>
			</c:choose>
		</c:forEach>
	...
	<c:if test="${pageModel.pageIndex < pageModel.pages}">
		<a class="next arrow" href="javascript:void(0);" onclick="javascript:doPage_goods('${pageModel.pageIndex + 1 }');"></a><!--下一页-->
	</c:if>
</div>
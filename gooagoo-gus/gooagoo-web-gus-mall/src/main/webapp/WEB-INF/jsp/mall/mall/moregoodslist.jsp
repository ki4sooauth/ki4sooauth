...
<input type="hidden" name="returnType" value="G" />
	...
	<ul id="convertGoodslist" class="product_list_box">
		<c:forEach items="${data }" var="uShopIntegral">
			<li class="content productC">
				...
				<p class="buyed "><a href="javascript:void(0);" onclick="getShippingAddress('${uShopIntegral.shopIntegralId }','${uShopIntegral.shopId }')" class="btnOrange">兑换</a><strong>已被兑换<samp>${uShopIntegral.convertCount }</samp>次</strong></p>
				<p class="shops">来自<span>${uShopIntegral.shopName }</span></p>
			</li>
		</c:forEach>
	</ul>
</c:if>

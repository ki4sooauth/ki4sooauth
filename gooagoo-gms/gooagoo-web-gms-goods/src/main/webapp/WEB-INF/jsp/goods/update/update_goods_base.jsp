	...
	<form method="post" id="goods_base">
		<ul>
			<li>
				...
				<select class="borderStyle text"  onchange="changeSelect();" style="display: none;"  name="entityId" id="shopEntityId">  
				 <c:forEach items="${shopEntityList }" var="item">
				 <option value="${item.shopEntityId }"  ${item.shopEntityId==goodsInfo.entityId ?'selected':""} >${item.shopEntityName }</option>
				 </c:forEach>
				   </select> 	   
				${shopEntityName}
			</li>
			<li><span class="tit_1">${shopVo.wordNames['gmse004']}</span><input class="borderStyle text" type="text"
				name="goodsName" value="${goodsInfo.goodsName }"/></li>
			<li><span class="tit_1">${shopVo.wordNames['gmse005']}</span><input class="borderStyle text"
				type="text" name="categoryLeafName" id="goods_category" readonly value="${goodsInfo.categoryLeafName }"  onclick="relateOne('J');"  />
			...
			<li>
				<span class="tit_1">${shopVo.wordNames['gmse006']}</span>
				<input class="borderStyle text" type="text" readonly name="brandName" value="${goodsInfo.brandName }"  id="goods_brand" onclick="relateOne('B');"/>
			...
			<li>
			<span class="tit_1">${shopVo.wordNames['gmse047']}</span>
			<input class="borderStyle text"  type="text" name="price" value="${goodsInfo.price }"/>
			</li>		

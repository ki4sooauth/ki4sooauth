...
<body>
	...
	<!--内容-->
				<div class="rightTitle">
					<span class="tit_1">${shopVo.wordNames['gmse104']}</span>
				</div>
				<div class="conditions_content">
					...
					<!-- 商品营销信息 -->
					<c:if test="${!empty marketing}">
					<%@include file="/WEB-INF/jsp/goods/view/view_marketing.jsp"%>
					</c:if>					
					<!-- 状态信息 -->
					<div class="title">${shopVo.wordNames['gmse105']}</div>
					<div class="productBuild">
						<ul>
							<li>
								<span class="tit_1">${shopVo.wordNames['gmse105']}</span>
								<c:if test="${goodsInfo.status == 'W' }">
									${shopVo.wordNames['gmse106']}
								</c:if>
								<c:if test="${goodsInfo.status == 'A' }">
									${shopVo.wordNames['gmse107']}
								</c:if>
								<c:if test="${goodsInfo.status == 'B' }">
									${shopVo.wordNames['gmse108']}
								</c:if>
								<c:if test="${goodsInfo.status == 'P' }">
									${shopVo.wordNames['gmse109']}
								</c:if>
								<c:if test="${goodsInfo.status == 'D' }">
									${shopVo.wordNames['gmse110']}
								</c:if>

							</li>
							<li>
								<span class="tit_1">${shopVo.wordNames['gmse111']}</span>
								${goodsInfo.auditNote }
							</li>
						</ul>
					</div>

...
public class GMSUtil
{
     ...
     * @return
     */
    public static GmsLoginInfo getGmsLoginInfoByWeb(HttpServletRequest request)
    {
    ...
    /**
     * 获取一个空对象（String字段值为""，Long和Integer字段值为0，Double为0.0，其他为null）
     * @param clazz
     * @return
     * @throws InstantiationException
     * @throws IllegalAccessException
     */
    public static <T> T getBlankObject(Class<T> clazz) throws InstantiationException, IllegalAccessException
    {
    ...
    {
        return GMSHttpUtils.transfer(code, request, dataClass);
    }

    /**
     * 往页面输出json格式结果（返回布尔类型公用调用）
     * @param isSuccess
     * @param resultCode
     * @param request
     * @param response
     * @throws Exception
     */
    public static void ajaxResult(boolean isSuccess, String resultCode, HttpServletResponse response) throws Exception
    {
        GAjax rv = getGAjaxByCode(isSuccess, resultCode);
    ...
    public static void ajaxSubmit(String interfaceCode, HttpServletRequest request, HttpServletResponse response) throws Exception
    {
        ...
        GAjax rv = getGAjaxByCode(respObj.getHead().isSuccess(), resultCode);
     ...
     * @return
     */
    public static GAjax getGAjaxByCode(boolean isSuccess, String resultCode)
    {
        ...
        GAjax rv = new GAjax(isSuccess, resultName);
        return rv;
    }
     ...
     * @return
     */
    public static String getWebUrlByAuthorityId(HttpServletRequest request, String authId)
    {
        ShopLoginInfo shopLoginInfo = (ShopLoginInfo) request.getAttribute("shopVo");
        String webLink = "";
        if (shopLoginInfo != null && !CollectionUtils.isEmpty(shopLoginInfo.getShopAuthList()))
        {
            List<ShopAuth> shopAuthList = shopLoginInfo.getShopAuthList();
            for (ShopAuth shopAuth : shopAuthList)
            {
                if (authId.equals(shopAuth.getAuthorityId()))
                {
                    webLink = shopAuth.getLink();
                    request.setAttribute("webLink", webLink);
                    break;
                }
            }
            if (!StringUtils.hasText(webLink))
            {
                GooagooLog.debug("跟据权限id获取web链接地址 没有找到对应的url！");
            }
        }
        else
        {
            GooagooLog.debug("跟据权限id获取web链接地址时，获取登录信息错误！");
        }
        return webLink;
    }

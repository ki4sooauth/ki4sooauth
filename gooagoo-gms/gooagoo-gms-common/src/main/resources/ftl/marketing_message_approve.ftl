...
<script type="text/javascript">
	function checkMessage(messageId,status){
		$(".inputBtn").attr("disabled","disabled");
		var url = "${basePath }activityCont.do?method=check";
		var data = "&id="+messageId+ "&note="+$("#note").val()+"&status="+status+"&channelCode=${channelCode}";
	    ...
	    <div>
	    	<div style="margin-top:10px;margin-bottom:8px">
      		...
      		<div style="margin:10px; text-align:center;">
      			<a href="#" onclick="checkMessage('${message.messageId}','Y');" style="width: 80px; height: 28px;line-height: 28px;" class="inputBtn blueBtn"><strong>${shopVo.wordNames['gmsc011']}</strong></a>
     			<span style="width:10px;">&nbsp;</span>
      			<a href="#" onclick="checkMessage('${message.messageId}','N');" style="width: 80px; height: 28px;line-height: 28px;" class="inputBtn blueBtn"><strong>${shopVo.wordNames['gmsc012']}</strong></a>
	 	 	</div>
    	</div>
	</div>
</body>

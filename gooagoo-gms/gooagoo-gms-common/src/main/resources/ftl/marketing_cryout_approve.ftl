...
<script type="text/javascript">
	function checkCryout(cryoutInfoId,status){
		$(".inputBtn").attr("disabled","disabled");
		var url = "${basePath }activityCont.do?method=check";
		var data = "&id="+cryoutInfoId+ "&note="+$("#note").val()+"&status="+status+"&channelCode=${channelCode}";
    	...
    	<div>
	    	<div style="margin-top:10px;margin-bottom:8px">
	      	...
	      	<div style="margin:10px; text-align:center;">
	      		<a href="#" onclick="checkCryout('${cryout.cryoutInfoId}','Y');" style="width: 80px; height: 28px;line-height: 28px;" class="inputBtn blueBtn"><strong>${shopVo.wordNames['gmsc011']}</strong></a>
	      		<span style="width:10px;">&nbsp;</span>
	      		<a href="#" onclick="checkCryout('${cryout.cryoutInfoId}','N');" style="width: 80px; height: 28px;line-height: 28px;" class="inputBtn blueBtn"><strong>${shopVo.wordNames['gmsc012']}</strong></a>
		  	</div>
    	</div>
	</div>
</body>

...
<script type="text/javascript">
	function checkEvent(eventId,status){
		$(".blueBtn").attr("disabled","disabled");
		var url = "${basePath }activityCont.do?method=check";
		var data = "&id="+eventId+ "&note="+$("#note").val()+"&status="+status+"&channelCode=${channelCode}";
	    ...
	    <div>
	    	<div style="margin-top:10px;margin-bottom:8px">
     		...
     		<div style="margin:10px; text-align:center;">
      			<a href="#" onclick="checkEvent('${event.eventId}','Y');" style="width: 80px; height: 28px;line-height: 28px;" class="inputBtn blueBtn"><strong>${shopVo.wordNames['gmsc011']}</strong></a>
      			<span style="width:10px;">&nbsp;</span>
      			<a href="#" onclick="checkEvent('${event.eventId}','N');" style="width: 80px; height: 28px;line-height: 28px;" class="inputBtn blueBtn"><strong>${shopVo.wordNames['gmsc012']}</strong></a>
	  		</div>
		</div>
	</div>
</body>

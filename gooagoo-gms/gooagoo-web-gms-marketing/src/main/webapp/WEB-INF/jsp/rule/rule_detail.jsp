...
<script type="text/javaScript">
	...
	var cbody = true;
	$(function(){
		var activityName = parent.$("#nextPage input[name='activityName']").val();
		var title = parent.$("#nextPage input[name='title']").val();
		$("#acName").append(activityName);
		$("#conName").append(title);
	})
</script>
</head>
<body>
    ...
    <div class="container">
      	<div class="article" style="width: 860px;">
      		<div class="memberPoup">
			    	...
			    	<c:if test="${detailType == 'release' }">
						...
						<div class="conditions_content">
							<div class="title">${shopVo.wordNames['gmsc250']}</div>
						  	<div class="plan_box">
						      	<p class="top_p" id="acName">
							      	<span><font style="color: #5B5F60;font-weight: bold;">${shopVo.wordNames['gmsc251']}：</font></span>
						      	</p>
						      	<p class="top_p" id="conName">
							      	<span><font style="color: #5B5F60;font-weight: bold;">${shopVo.wordNames['gmsc252']}：</font></span>
						      	</p>
					      	</div>
						</div>
			    	</c:if>
			    	<c:if test="${detailType == 'ruleDetail' }">
						...
						<div class="conditions_content">
						 	<div class="title">${shopVo.wordNames['gmsc075']}</div>
						  	<div class="behavior_quality noborder">
						    	<dl>
						        	<dd><span>${shopVo.wordNames['gmsc052']}</span><font>${info.ruleName}</font></dd>
						        	<dd><span>${shopVo.wordNames['gmsc053']}</span>
						        		<c:forEach items="${rule_type}" var = "type">
						        			<c:if test="${info.ruleType==type.key}"><font>${type.value}</font></c:if>	
						               	</c:forEach>
						        	</dd>
						        	<dd><span>${shopVo.wordNames['gmsc059']}</span>
					        			<c:forEach items="${rule_result_type}" var = "type">
						        			<c:if test="${result.ruleResultType==type.key}"><font>${type.value}</font></c:if>	
						               	</c:forEach>
						        	</dd>
						        	<c:if test="${result.ruleResultType=='0'}">
							        	<dd id="r0"><span>${shopVo.wordNames['gmsc060']}</span><font>${result.ruleResultValue}${shopVo.wordNames['gmsc066']}</font></dd>
						        	</c:if>	
						        	<c:if test="${result.ruleResultType=='1'}">
							        	<dd id="r1"><span>${shopVo.wordNames['gmsc065']}</span><font>${result.ruleResultName}</font></dd>
							        </c:if>	
						        	<c:if test="${result.ruleResultType=='2'}">
							        	<dd id="r2"><span>${shopVo.wordNames['gmsc061']}</span><font>${result.ruleResultValue}${shopVo.wordNames['gmsc067']}</font></dd>
							        </c:if>
						        	<c:if test="${result.ruleResultType=='3'}">
							        	<dd id="r3"><span>${shopVo.wordNames['gmsc062']}</span><font>${result.ruleResultValue}${shopVo.wordNames['gmsc068']}</font></dd>
						        	</c:if>
						        	<c:if test="${result.ruleResultType=='4'}">
							        	<dd class="bottom_box" id="r4"><span>${shopVo.wordNames['gmsc063']}</span><font>${result.ruleResultName}</font></dd>
			                        </c:if>
			                        <c:if test="${result.ruleResultType=='5'}">
							        	<dd class="bottom_box" id="r5"><span>${shopVo.wordNames['gmsc064']}</span><font>${result.ruleResultName}</font></dd>
			                        </c:if>
						...
						<c:if test="${not empty ziran}">
							<ul style="width: 750px; height: 20px;">
								...
								<c:if test="${not empty natural.account}">
				         	 		<li style="width: 280px;"><span class="num">${shopVo.wordNames['gmsc084']}</span>&nbsp;<font>${natural.account}</font></li>
				         	 	</c:if>
				         	 	<c:if test="${not empty natural.sex}">
									<li style="width: 200px;"><span>${shopVo.wordNames['gmsc085']}</span><font>
								...
								<c:if test="${not empty hist}">
									<dl class="action_${hist.actionType}">
								        ...
								        <c:if test="${not empty hist.timeMin || not empty hist.totalTimeMin}">
								        	<dd><span>${shopVo.wordNames['gmsc126']}</span>
									        	<c:if test="${not empty hist.timeMin}">
									        		<font>${shopVo.wordNames['gmsc094']}：</font>
										        	<font>${hist.timeMin}-${hist.timeMax}&nbsp;${shopVo.wordNames['gmsc069']}</font>
									        	</c:if>
									        	<c:if test="${not empty hist.totalTimeMin}">
									        		<font>${shopVo.wordNames['gmsc095']}：</font>
									        		<font>${hist.totalTimeMin}-${hist.totalTimeMax}&nbsp;${shopVo.wordNames['gmsc069']}</font>
									        	</c:if>
								        	</dd>
								      	</c:if>
									    ...
									    <c:if test="${not empty hist.marketingGoodsCategoryName}">
						           		...
						           		<c:if test="${not empty hist.durationMin}">
											<dd id="isShowDuration"><span>${shopVo.wordNames['gmsc116']}</span>
												<font>${hist.durationMin}-${hist.durationMax}&nbsp;${shopVo.wordNames['gmsc128']}</</font>
										    </dd>
										</c:if>
						           		...
						           		<c:if test="${not empty hist.cmsContent}">
									        <dd id="isShowVb">
												<div class="bottom_box"><span>${shopVo.wordNames['gmsc133']}</span>
										           	<ul class="tag">
								...
								<dl class="current_action" id="${curr.actionType}">
							        ...
							        <c:if test="${not empty curr.timeMin || not empty curr.totalTimeMin}">
							        	...
							        	<c:if test="${not empty curr.timeMin}">
							        		<font>${shopVo.wordNames['gmsc094']}：</font>
								        	<font>${hist.timeMin}-${curr.timeMax}&nbsp;${shopVo.wordNames['gmsc069']}</font>
							        	</c:if>
							        	<c:if test="${not empty curr.totalTimeMin}">
							        		<font>${shopVo.wordNames['gmsc095']}：</font>
							        		<font>${curr.totalTimeMin}<-${curr.totalTimeMax}&nbsp;${shopVo.wordNames['gmsc069']}</font>
							        	</c:if>
							      	</c:if>
							        <c:if test="${not empty curr.marketingGoodsCategoryName || not empty curr.marketingGoodsBrandName || not empty curr.marketingGoodsName}">
					          		...
					          		<c:if test="${not empty curr.marketingActivityName}">
					           		...
					           		<c:if test="${not empty curr.serverToolsName}">
										<dd id="isShowServerTools">
											<div class="bottom_box"><span>${shopVo.wordNames['gmsc104']}具</span>
									           	<ul class="tag">
					          		...
					          		<c:if test="${not empty curr.positionName}">
					           		...
					           		<c:if test="${not empty curr.durationMin}">
										<dd id="isShowDuration"><span>${shopVo.wordNames['gmsc116']}</span>
											<font>${curr.durationMin}-${curr.durationMax}&nbsp;${shopVo.wordNames['gmsc128']}</font>
									    </dd>
									</c:if>
				...
				<a href="javascript:void(0)" class="blueBtn savebtn" onclick="parent.$.fancybox.close();" style="margin-bottom:20px;">${shopVo.wordNames['gmsc003']}</a>
       		</div>
     	</div>
	</div>
</body>

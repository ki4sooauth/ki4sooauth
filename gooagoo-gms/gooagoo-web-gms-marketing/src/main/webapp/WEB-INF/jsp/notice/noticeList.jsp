		...
		<c:if test="${not empty pm.result}">
			<c:forEach var="itemChild" items="${pm.result}" varStatus="xh">
			    ...
			    <td>${pm.pageSize*(pm.pageIndex-1)+xh.index+1}</td>
					...
					<td>
					   <check:hasAuthority authorityID='13070201'> 
					 <c:if test="${itemChild.publishStatus=='W'||itemChild.publishStatus=='B'}">
		                	<a href="javascript:formNotice('${itemChild.noticeInfoId}');" class="handle pencil" title="${shopVo.wordNames['gmsc013']}"></a>
		                </c:if>
	                   </check:hasAuthority>
	                 <check:hasAuthority authorityID='13070203'> 
	                  <c:if test="${itemChild.publishStatus=='A'}">	                    
	                    <a href="javascript:formReleaseCont('${itemChild.noticeInfoId}','${itemChild.noticeTitle}','${itemChild.activityName}','<fmt:formatDate value="${itemChild.activeStartTime}" type="both" pattern="yyyy-MM-dd" />','<fmt:formatDate value="${itemChild.activeEndTime}" type="both" pattern="yyyy-MM-dd" />');" class="handle issue" title="${shopVo.wordNames['gmsc017']}" ></a>
	                   </c:if>	
	                  </check:hasAuthority>					
					 <a href="javascript:void(0)" onclick="noticeDetail('${itemChild.noticeInfoId}');" title="${shopVo.wordNames['gmsc015']}" class="detailMark" ></a>        
					 ...
					 <check:hasAuthority authorityID='13070205'> 
					 <a href="javascript:deleteNotice('${itemChild.noticeInfoId}');" title="${empty shopVo.wordNames['gmsd046'] ? '删除' : shopVo.wordNames['gmsd046']}" class="handle del"></a>
					 </check:hasAuthority>					
					</td>
				</tr>
			</c:forEach>

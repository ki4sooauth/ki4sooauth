...
<title>${shopVo.wordNames['gmsc016']}</title>
...
<script type="text/javascript">
	...
	//审核规则
	function checkMemberCard(id,status){
		$(".blueBtn").attr("disabled","disabled");
		var note = $("textarea[name='note']").val();
		var url = basePath + "memberCard.do?method=check";
		var data = "&cardId="+id + "&status="+status+"&note="+note;
	...
	<div style="height:auto; padding: 5px 25px;" >
	    <div>
			...
			<div style="margin:10px; text-align:center;">
				<a href="#" onclick="checkMemberCard('${cardId}','Y');" style="width: 80px; height: 28px;line-height: 28px;" class="inputBtn blueBtn"><strong>${shopVo.wordNames['gmsc011']}</strong></a>
				<span style="width:10px;">&nbsp;</span>
				<a href="#" onclick="checkMemberCard('${cardId}','N');" style="width: 80px; height: 28px;line-height: 28px;" class="inputBtn blueBtn"><strong>${shopVo.wordNames['gmsc012']}</strong></a>
			</div>
		</div>
	</div>
</body>

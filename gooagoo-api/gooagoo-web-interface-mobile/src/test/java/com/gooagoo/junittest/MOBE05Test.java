...
public class MOBE05Test extends TestCase
{

    String interfaceCode = "mobe05";//接口编码
    ...
    public void testMacIsNull()
    {
        ...
        list.add(new BasicNameValuePair("goodsidlist", ""));

        list.add(new BasicNameValuePair("couponid", ""));
        list.add(new BasicNameValuePair("tableno2d", ""));
        SubmitOrderFormRoot root = new SubmitOrderFormRoot();
        root = (SubmitOrderFormRoot) HttpClientUtils.HttpPost(list, root, this.interfaceCode);
    ...
    public void testGoodsidlistIsNull()
    {
        List<NameValuePair> list = new ArrayList<NameValuePair>();//传入参数
        list.add(new BasicNameValuePair("mac", ""));
        list.add(new BasicNameValuePair("userid", ""));
        ...
        SubmitOrderFormRoot root = new SubmitOrderFormRoot();
        root = (SubmitOrderFormRoot) HttpClientUtils.HttpPost(list, root, this.interfaceCode);
        Assert.assertEquals(MessageTestConst.MOBILE_PARAMETER_GOODSIDLIST_IS_NULL, root.getMsg().trim());
    }
    ...
    public void testSuccess()
    {
        ...
        SubmitOrderFormRoot root = new SubmitOrderFormRoot();
        root = (SubmitOrderFormRoot) HttpClientUtils.HttpPost(list, root, this.interfaceCode);
        Assert.assertEquals("手机提交点菜单信息成功", root.getMsg().trim());
    }
}

...
@Service("gtsc02")
public class GTSC02ServiceImpl implements ItransService
{
    @Autowired
    ...
    public String doItrans(HttpServletRequest request) throws Exception
    {
        ...
        try
        {
            ...
            //打印log
            GooagooLog.info(InterfaceUtils.getLogMsg(request, "gtsc02"));
            //校验入参
            ...
            root = this.orderManageTransService.getUserOrderInfo(shopentityid, scardno, ctimestamp);
            root.setResult("true");
            root.setMsg("查询用户订单信息成功");
        }
